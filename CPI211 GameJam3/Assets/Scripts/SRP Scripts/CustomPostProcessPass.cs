...
{
    //The rendering pass for the ben day bloom effect
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        using (new ProfilingScope(cmd, new ProfilingSampler("Custom Post Processing Effects")))
        {
            //bloom pass goes first
            SetupBloom(cmd, m_CameraColorTarget);

            //Composite value setup
    ...
    private void SetupBloom(CommandBuffer cmd, RTHandle source)
    {
        ...
        float thresholdKnee = threshold * 0.5f; //Hardcoded for now
        ...
        {
            //Two pass gaussian blur, uses mipUp as a temporary target
            //First past is 2x downsample with 9-tap gaussian
            //Second pass is a 9-tap gaussian using a 5-tap filter and bilinear filtering
        ...
        //Upsample (Bilinear by default, with HQ filtering as bicubic)
